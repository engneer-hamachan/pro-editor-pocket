# SDカードにapp.rbのtdeck plus上で動くアプリケーション(main/mrblib/app.rb)で書いてるコードを保存したい

## Usage(アプリケーションから)
- SDCard.saveを実行したら、書いてるコードがSDカードに保存される
- SDCard.loadを実行したら、保存したコードが書いてるコードに展開される

## design
- components/picoruby-tft の実装を参考にmrubycバインディングを作成する
    - mrbgems名称はpicoruby-sdcard
    - app.rbからは require 'sdcard'で使える
    - メソッドはSDCard.saveとSDCard.loadのみ
    - components/picoruby-esp32/build_config/xtensa-esp.rb や components/picoruby-esp32/CMakeLists.txt 等で呼び出せるように設定する
...
## tdeck plusのexmaple
- ~/apps/T-Deck/examples/SD/SD.ino
- ~/apps/T-Deck/examples/SD/utilities.h
